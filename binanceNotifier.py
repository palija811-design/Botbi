...
def get_current_price(pair):
    """Precio actual desde Binance. pair viene como BTC/USDT -> BTCUSDT."""
    try:
        symbol = pair.replace("/", "")
...
def get_ticker_24h(pair):
    ...
    try:
        # Datos 24h desde Binance (un solo call da todo, quoteVolume ya en USDT)
        symbol = pair.replace('/', '')
        ...
        vol_24h_token = float(t['volume'])          # volumen en token base
        vol_24h_usd   = float(t['quoteVolume'])     # volumen en USDT (≈USD) — directo y preciso
...
function krakenUrl(pair) {
  var s = pair.replace('/', '_');
  return 'https://www.binance.com/en/trade/' + s + '?type=spot';
}
...
# ─────────────────────────────────────────────
# CONEXIÓN BINANCE — top 200 pares USDT por volumen
# ─────────────────────────────────────────────
import websocket as _ws_module

TOP_N_PARES = 200
_binance_symbols = []          # ['BTCUSDT', 'ETHUSDT', ...]
_symbol_to_pair = {}           # 'BTCUSDT' -> 'BTC/USDT'

def get_top_binance_pairs(n=TOP_N_PARES):
    """Obtiene los top N pares USDT por volumen 24h desde el REST de Binance."""
    try:
        r = requests.get("https://api.binance.com/api/v3/ticker/24hr", timeout=20)
        data = r.json()
        usdt = [d for d in data if d["symbol"].endswith("USDT")]
        # Excluir stablecoins y apalancados (UP/DOWN/BULL/BEAR)
        excluir = ("UPUSDT","DOWNUSDT","BULLUSDT","BEARUSDT")
        stables = ("USDCUSDT","FDUSDUSDT","TUSDUSDT","BUSDUSDT","DAIUSDT","EURUSDT","GBPUSDT","USDPUSDT")
        usdt = [d for d in usdt if not d["symbol"].endswith(excluir) and d["symbol"] not in stables]
        usdt.sort(key=lambda x: float(x["quoteVolume"]), reverse=True)
        top = usdt[:n]
        symbols = [d["symbol"] for d in top]
        print(f"Top {len(symbols)} pares Binance por volumen obtenidos")
        return symbols
    except Exception as e:
        print(f"Error obteniendo pares Binance: {e}")
        return []
...
def get_current_price_binance(symbol):
    """Precio actual de un símbolo Binance (para tracking)."""
    try:
...
def createTGmessage(tradeDF, pair, volInEUR, priceDiff, wsnames, pairs, ticker=None, change_7d=None, ai_scores=None):
    ...
    token = pair.split("/")[0]
    base = pair.split("/")[1] if "/" in pair else "USDT"
    ...
    # Enlace a Binance (trade spot)
    symbol_binance = pair.replace("/", "_")
    url = f'[{pair}](https://www.binance.com/en/trade/{symbol_binance}?type=spot)'
    terceraLinea = f"\n{url}"
    ...
    if ai_scores:
        ...
        quality = "🟢" if score_final >= 8 else "🟡" if score_final >= 6 else "🔴"
        ...
        septimaLinea = ""
    return f"{primeraLinea} {segundaLinea} {terceraLinea}{sextaLinea}{quintaLinea}{septimaLinea}"
...
def twilio_llamada(pair, priceDiff, volInEUR):
    """Dispara una llamada de alerta via Twilio."""
    try:
        r = requests.get(
...
# Acumulador de trades por símbolo para detectar movimientos bruscos
_trade_buffer = {}       # symbol -> list of (price, qty, time, side)
_buffer_lock = threading.Lock()
_last_price = {}         # symbol -> ultimo precio visto

def procesar_trade(symbol, precio, cantidad, es_venta, ts_ms):
    """Acumula trades y detecta movimientos de ballena en ventanas cortas."""
    pair = _symbol_to_pair.get(symbol, symbol_to_pair(symbol))
    usd = precio * cantidad
    side = "s" if es_venta else "b"

    # Detección directa: una sola operación grande ya es señal de ballena
    # Guardamos el precio para calcular el movimiento
    with _buffer_lock:
        prev = _last_price.get(symbol)
        _last_price[symbol] = precio

    if prev is None:
        return
    priceDiff = abs((precio - prev) * 100 / prev)

    # Filtro: movimiento > 2% Y volumen operación > 15.000 USD
    if priceDiff > 2 and usd > 15000:
        ...
        tradeDF = pd.DataFrame([{
            "price": precio, "volume": cantidad, "time": ts_ms/1000,
            "side": side, "orderType": "m", "misc": ""
        }])
        entry_price = precio
        volInEUR = usd  # ya en USD (USDT)
        ...
        if vol_24h is None:
            print(f"⏭ [BINANCE] {pair} omitido — sin vol 24h")
            return
        if vol_24h < 50000:
        ...
        def send_ai_score_reply(pair=pair, priceDiff=priceDiff, volInEUR=volInEUR,
                                side=side, ticker=ticker, c7d=c7d, msg_id=msg_id, signal_id=signal_id):
            if not ANTHROPIC_KEY:
                return
            try:
                ...
                url = f"https://api.telegram.org/bot{bot_token}/sendMessage"
                payload = {
                    "chat_id": bot_chatID, "parse_mode": "Markdown",
                    "text": score_msg, "link_preview_options": {"is_disabled": True}
                }
                if msg_id:
        ...
        if count_recent >= RAFAGA_MIN_COUNT:
            ...
            print(f"📊 Twilio check: {pair} | señales: {count_recent} | score: {last_score}")
            if last_score is None or last_score >= 7:
                threading.Thread(target=twilio_llamada, args=(pair, priceDiff, volInEUR), daemon=True).start()


def tradeLoop_binance(symbols):
    """Conecta al WebSocket de Binance con streams combinados de aggTrade."""
    # Binance permite hasta ~1024 streams por conexión combinada
    ...
    def on_message(ws, message):
        try:
            ...
            es_venta = data.get("m", False)  # m=True: comprador es maker → venta agresiva
            ts_ms = data.get("T", 0)
            if symbol and precio > 0:
                procesar_trade(symbol, precio, cantidad, es_venta, ts_ms)
        except Exception as e:
    ...
    def on_close(ws, code, msg):
        print(f"[BINANCE] WS cerrado (code={code}). Reconectando en 5s...")

    def on_open(ws):
        print(f"[BINANCE] Conectado — {len(symbols)} pares suscritos")

    while True:
        try:
            ws = _ws_module.WebSocketApp(
                url, on_open=on_open, on_message=on_message,
                on_error=on_error, on_close=on_close
            )
            ws.run_forever(ping_interval=180, ping_timeout=10)
        except Exception as e:
            print(f"[BINANCE] Excepción en run_forever: {e}")
        print("[BINANCE] Reintentando conexión en 5s...")
        time.sleep(5)
...
def connectTradeWS():
    ...
    # Evaluación de predicciones cada hora
    def loop_evaluacion():
        while True:
            time.sleep(3600)
            try:
    ...
    print("📊 Hilo de evaluación arrancado")

    # Obtener top 200 pares por volumen
    global _binance_symbols, _symbol_to_pair
    _binance_symbols = get_top_binance_pairs(TOP_N_PARES)
    if not _binance_symbols:
        print("❌ No se pudieron obtener pares de Binance. Reintentando en 30s...")
        time.sleep(30)
        _binance_symbols = get_top_binance_pairs(TOP_N_PARES)
    _symbol_to_pair = {s: symbol_to_pair(s) for s in _binance_symbols}
    print(f"Total pares Binance: {len(_binance_symbols)}")
    telegram_bot_sendtext(f"\U0001F7E1 Bot Binance levantado — {len(_binance_symbols)} pares (top {TOP_N_PARES} por volumen)")

    # Refrescar la lista de top pares cada 12h en background
    def loop_refresh_pares():
        global _binance_symbols, _symbol_to_pair
        while True:
            time.sleep(12 * 3600)
            nuevos = get_top_binance_pairs(TOP_N_PARES)
            if nuevos:
                _binance_symbols = nuevos
                _symbol_to_pair = {s: symbol_to_pair(s) for s in nuevos}
                print(f"♻ Lista de pares actualizada: {len(nuevos)}")
    threading.Thread(target=loop_refresh_pares, daemon=True).start()

    # Hilo principal del WebSocket con auto-reinicio
    t_ws = threading.Thread(target=tradeLoop_binance, args=(_binance_symbols,), daemon=True)
    t_ws.start()
    print("Hilo Binance WS arrancado")

    while True:
        time.sleep(60)
        if not t_ws.is_alive():
            print("⚠️ Hilo Binance muerto — reiniciando...")
            t_ws = threading.Thread(target=tradeLoop_binance, args=(_binance_symbols,), daemon=True)
            t_ws.start()
